(function(){
 ...
 var navLinks='<a href="#servicos">'+esc(lab.servicos||'Serviços')+'</a>'+
  ...
  (L.mvv||L.valores?'<a href="#valores">Missão</a>':'')+
 ...
 h+='<section id="servicos"><div class="w"><div class="lab">'+esc(lab.servicos||'Serviços')+'</div><h2>'+esc(L.tituloServicos||'O que fazemos')+'</h2>'+(L.planos&&L.planos.length?'<button class="btn b2" id="planosBtn" type="button" style="margin-bottom:32px">Ver planos</button>':'')+'<div class="svc">'+L.servicos.map(function(s,i){return '<div class="row"><i>0'+(i+1)+'</i><h3>'+esc(s.t)+'</h3><p>'+esc(s.d||'')+'</p><a class="btn b2" href="'+wa+'">'+esc(cta)+'</a></div>'}).join('')+'</div></div></section>';
 if(L.planos&&L.planos.length){h+='<div class="modal" id="planosModal" style="display:none"><div class="modal-card"><button class="modal-x" id="planosClose" type="button" aria-label="Fechar">&times;</button><div class="lab">Planos</div><h2 style="margin-bottom:24px">Escolha o seu plano</h2><div class="plan-grid">'+
  L.planos.map(function(p){var waP='https://wa.me/'+L.whatsapp.numero+'?text='+encodeURIComponent('Olá! Tenho interesse no plano '+p.nome+' e gostaria de agendar uma aula experimental.');return '<div class="plan-card'+(p.cor?' plan-'+p.cor:'')+'"><h3>'+esc(p.nome)+'</h3><div class="plan-price">'+(p.de?'<span class="plan-old">R$ '+esc(p.de)+'</span>':'')+(p.preco?'R$ '+esc(p.preco):esc(p.precoTxt||'Consultar'))+(p.periodo?'<span>'+esc(p.periodo)+'</span>':'')+'</div>'+(p.desc?'<p>'+esc(p.desc)+'</p>':'')+'<a class="btn b1" href="'+waP+'">Quero esse</a></div>'}).join('')+
  '</div></div></div>';}
 ...
 h+='<div class="ct" id="contato"><div class="w"><div><div class="lab" style="opacity:.8">Contato</div><h2>'+esc(L.contato.titulo)+'</h2><p>'+esc(L.contato.texto)+'</p><a class="btn b1" href="'+wa+'">'+esc(L.ctaWhats||'Falar pelo WhatsApp')+'</a></div><div><div class="l"><small>Endereço</small><span>'+esc(L.endereco[0])+'<br>'+esc(L.endereco[1])+'</span></div><div class="l"><small>WhatsApp</small><span>'+esc(L.whatsapp.display)+'</span></div>'+(L.horarios?'<div class="l"><small>Horário</small><span>'+esc(L.horarios)+'</span></div>':'')+(L.mapaImg?'<div class="mapa-estatico"><img src="'+L.mapaImg+'" alt="Mapa da região"><div class="mapa-pin" style="left:'+(L.mapaPin&&L.mapaPin.x||'50%')+';top:'+(L.mapaPin&&L.mapaPin.y||'50%')+'"><small>'+esc(L.nome)+'</small><span class="pulse"></span><span class="dot"></span></div></div>':(L.mapa?'<div class="ct-map" style="display:flex;align-items:center;justify-content:center;background:var(--alt)"><a class="btn b2" href="https://www.google.com/maps/search/?api=1&query='+encodeURIComponent(L.endereco.join(', '))+'" target="_blank" rel="noopener">Ver no Google Maps</a></div>':''))+'</div></div></div></main>';
 h+='<footer><div class="w"><span>© '+new Date().getFullYear()+' '+esc(L.nome)+'. Todos os direitos reservados.</span><a href="https://criativamentedigital.com.br/sites" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;gap:6px;opacity:.7;font-size:12px">Desenvolvido por <img src="img/cd-logo.png" alt="Criativamente Digital" style="width:16px;height:16px;object-fit:contain"> Criativamente Digital</a></div></footer><a class="wa" href="'+wa+'" aria-label="WhatsApp">'+WAI+'</a>';
 ...
 if(L.planos&&L.planos.length){
  var pModal=document.getElementById('planosModal');
  var pOpen=function(){pModal.style.display='flex'};
  var pClose=function(){pModal.style.display='none'};
  document.getElementById('planosBtn').addEventListener('click',pOpen);
  document.getElementById('planosClose').addEventListener('click',pClose);
  pModal.addEventListener('click',function(e){if(e.target===pModal)pClose()});
